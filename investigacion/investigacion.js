(() => {
  ...
  function renderResult(r){
    ...
    $('#observations-result').innerHTML=(r.observations||[]).map(o=>`<div class="accordion-item open"><div class="accordion-head"><span class="badge ${o.severity==='Crítico'?'badge-danger':o.severity==='Alto'?'badge-warning':'badge-info'}">${o.severity}</span><div class="grow"><strong>${o.section}: ${o.title}</strong><div class="small muted">${o.page||''}</div></div></div><div class="accordion-body"><dl>${o.original?`<dt>Texto observado</dt><dd>${o.original}</dd>`:''}<dt>Problema</dt><dd>${o.problem||''}</dd><dt>Por qué importa</dt><dd>${o.why||''}</dd><dt>Cómo corregir</dt><dd>${o.fix||''}</dd>${o.proposal?`<dt>Propuesta</dt><dd>${o.proposal}</dd>`:''}</dl></div></div>`).join('') || '<p class="muted">Sin observaciones relevantes.</p>';
    show('#view-result');
  }

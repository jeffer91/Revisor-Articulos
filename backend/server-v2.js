...
async function attemptProvider(job,model,clean,successes,failures,lane,automatic){
  ...
  await setProvider(job,model,'Procesando',`Carril: ${lane.label}`,null,lane);
  try{
    const article=hybrid.articleForLane(clean,model,lane);
    const prompt=hybrid.buildSpecializedPrompt(article,model,lane,automatic);
    const result=await ai.callModel(model,prompt),latency=Date.now()-started;
    successes.push({model,result,lane});
    job.reviewers=successes.length;
    job.step=Math.min(6,2+successes.length);
    ...
    failures.push({model:model.name,provider:model.provider,lane:lane.label,status,message});
...
async function runReview(job,articleText){
  try{
    ...
    const automatic=hybrid.analyzeAutomatic(clean);
    job.step=2;
    ...
    for(const m of active){
      ...
      if(problem){
        job.providerStatuses[m.id]={name:m.name,provider:m.provider,status:'Sin configurar',message:problem,updatedAt:new Date().toISOString()};
        failures.push({model:m.name,provider:m.provider,status:'Sin configurar',message:problem});
        await store.updateModelReviewHealth(m,'Sin configurar',problem,job.id);
      }else{
        job.providerStatuses[m.id]={name:m.name,provider:m.provider,status:'No participó',message:'',updatedAt:new Date().toISOString()};
        candidates.push(m);
      }
    }
    job.failures=failures;
    ...
    if(candidates.length<3){
      job.status='incomplete';job.step=8;
      job.message=`Solo hay ${candidates.length} proveedores configurados y disponibles. Se requieren al menos 3. Tu intento no fue descontado.`;
    ...
    while(unresolved.length&&cursor<candidates.length){
      ...
      for(const lane of unresolved){
        while(cursor<candidates.length&&used.has(candidates[cursor].id))cursor++;
        const model=candidates[cursor++];
        if(!model)break;
        used.add(model.id);
        batch.push(attemptProvider(job,model,clean,successes,failures,lane,automatic));
      }
    ...
    if(unresolved.length||successes.length<3){
      job.status='incomplete';job.step=8;
      job.message=`Solo respondieron ${successes.length} proveedores de forma válida. Se requieren 3 carriles académicos completos. Tu intento no fue descontado.`;
      await store.persistJob(job);return;
    }

    job.step=6;await store.persistJob(job);
    job.step=7;await store.persistJob(job);
    job.result=hybrid.consolidateHybrid(successes,job.file,job.cedula,automatic);
    job.result.id=job.id;
    job.status='complete';job.step=8;job.consumesAttempt=true;
    job.message='Revisión híbrida completada correctamente.';
    await store.persistJob(job);
  }catch(err){
    job.status='failed';job.step=8;job.message=String(err.message||err);job.consumesAttempt=false;
...
const server=http.createServer(async(req,res)=>{
  ...
  try{
    if(req.method==='GET'&&url.pathname==='/health'){
      const models=await store.loadModels(),active=models.filter(m=>m.state==='Activa'),ready=active.filter(m=>!store.configurationProblem(m));
      return json(res,200,{ok:true,service:'Revisor Artículos API',engine:'hybrid-v1',database:true,models:models.length,active:active.length,ready:ready.length,providers:[...new Set(models.map(m=>m.provider))],time:new Date().toISOString()},origin);
    }
    ...
    if(url.pathname.startsWith('/admin/')){
      ...
      const grant=url.pathname.match(/^\/admin\/students\/(\d{10})\/grant$/);
      if(req.method==='POST'&&grant){const body=await readJson(req);return json(res,200,await store.grantAttempts(grant[1],body.count),origin)}
      const restore=url.pathname.match(/^\/admin\/reviews\/([0-9a-f-]+)\/restore$/i);
      if(req.method==='POST'&&restore){await store.restoreAttempt(restore[1]);return json(res,200,{ok:true},origin)}
      const match=url.pathname.match(/^\/admin\/models\/([^/]+)(\/test)?$/);
      if(match){
        const id=decodeURIComponent(match[1]),model=(await store.loadModels()).find(m=>m.id===id);
      ...
      if(req.method==='POST'&&url.pathname==='/admin/models'){
        const body=await readJson(req),id=body.id||`model-${Date.now()}`,saved=await store.saveModelConfig({...body,id},body.apiKey||'');
        return json(res,200,store.cleanModel(saved),origin);
      }
      return json(res,404,{message:'Ruta administrativa no encontrada.'},origin);
    }

    const state=url.pathname.match(/^\/students\/(\d{10})\/state$/);
    if(req.method==='GET'&&state)return json(res,200,await store.getStudentState(state[1]),origin);
    ...
    if(req.method==='POST'&&url.pathname==='/reviews'){
      ...
      const student=await store.getStudentState(cedula);
      if(student.available<=0)return json(res,403,{message:'No tienes revisiones disponibles.'},origin);
      ...
      return json(res,202,{id,status:'processing',engine:'hybrid-v1'},origin);
    }
    ...
    if(req.method==='GET'&&status){
      ...
      return json(res,200,{id:job.id,status:job.status,step:job.step,reviewers:job.reviewers,message:job.message||'',engine:'hybrid-v1'},origin);
    }

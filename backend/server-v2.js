...
function validateLaneCategories(result,lane){
  const rows=Array.isArray(result?.json?.categories)?result.json.categories:[];
  const names=rows.filter(Array.isArray).map(r=>String(r[0]||'').trim().toLowerCase());
  const missing=lane.categories.filter(name=>!names.includes(String(name).toLowerCase()));
  if(missing.length)throw new Error(`Respuesta incompleta para el carril. Faltan: ${missing.join(', ')}`);
}

...
function collectWindows(text,terms,limit){
  ...
  add(0,Math.min(source.length,4500),'inicio del artículo');
  for(const term of terms){let from=0,hits=0;while(hits<2){const idx=low.indexOf(term.toLowerCase(),from);if(idx<0)break;add(idx-1800,idx+4200,term);from=idx+term.length;hits++;if(windows.join('').length>=limit)break}if(windows.join('').length>=limit)break}
...
function providerContextBudget(model){
  const p=lower(`${model?.provider||''} ${model?.model||''}`);
  if(/groq|cerebras/.test(p))return 12000;
  if(/mistral|cloudflare/.test(p))return 16000;
  if(/nvidia|public ai|openrouter/.test(p))return 20000;
  return 22000;
}

function articleForLane(text,model,lane,overrideLimit=null){
  const configured=Math.max(12000,Number(model.maxInputChars)||90000);
  const laneTarget=lane.id==='methodology-analysis'?22000:lane.id==='results-closure'?20000:18000;
  const requested=Number(overrideLimit)>0?Number(overrideLimit):laneTarget;
  const laneLimit=Math.max(7000,Math.min(configured,providerContextBudget(model),requested));
...
function missingLaneMicrocriteria(result,lane){
  const json=result?.json||{},ids=new Set((Array.isArray(json.microcriteria)?json.microcriteria:[]).map(m=>String(m?.id||'').trim()));
  return laneMicrocriteria(lane).filter(m=>!ids.has(m.id));
}

function validateLaneResponse(result,lane){
  const missingMicro=missingLaneMicrocriteria(result,lane);
...
function buildMissingMicrocriteriaPrompt(articleText,model,lane,missing){
  const requested=missing.map(m=>`- ${m.id} | ${m.category} | ${m.label} | ${m.weight} pts`).join('\n');
  const system=`Completa ÚNICAMENTE los microcriterios faltantes de una revisión académica de titulación.
Carril: ${lane.label}.
Modelo configurado como: ${model.reviewType||'General'}.

MICROCRITERIOS FALTANTES:
${requested}
...
REGLAS:
- Evalúa solo los IDs solicitados.
- Usa evidencia real del artículo.
- No inventes datos, fuentes, páginas ni procedimientos.
- Si existe evidencia parcial, usa 25%, 50% o 75%; reserva 0% para ausencia real o incompatibilidad clara.
- Devuelve SOLO JSON válido con esta forma:
{"microcriteria":[{"id":"id exacto","status":"Cumple|Parcial alto|Parcial|Parcial bajo|No cumple","evidence":"evidencia breve","appliedAs":"criterio literal o equivalente aplicado"}]}
- No agregues categorías, observaciones ni explicaciones fuera del JSON.`;
  return {system,user:`<ARTICULO>\n${articleText}\n</ARTICULO>`};
}

function mergeLaneRepairResult(baseResult,repairResult){
  const baseJson={...(baseResult?.json||{})},repairJson=repairResult?.json||{};
  const map=new Map((Array.isArray(baseJson.microcriteria)?baseJson.microcriteria:[]).map(m=>[String(m?.id||'').trim(),m]));
  for(const m of Array.isArray(repairJson.microcriteria)?repairJson.microcriteria:[]){
    const id=String(m?.id||'').trim();if(id)map.set(id,m);
  }
  return {...baseResult,json:{...baseJson,microcriteria:[...map.values()]},repaired:true};
}
...
module.exports={REVIEW_LANES,MICROCRITERIA,analyzeAutomatic,articleForLane,buildSpecializedPrompt,buildMissingMicrocriteriaPrompt,missingLaneMicrocriteria,mergeLaneRepairResult,validateLaneResponse,getCriticalCandidates,verifyCriticalCandidates,consolidateHybrid};

...
async function runReview(job,articleText){
  try{
    ...
    job.result=hybrid.consolidateHybrid(successes,job.file,job.cedula,automatic,criticalConfirmations);job.result.id=job.id;job.status='complete';job.step=8;job.consumesAttempt=true;job.message='Revisión por microcriterios completada correctamente.';await store.persistJob(job);
...
const server=http.createServer(async(req,res)=>{
  ...
  try{
    if(req.method==='GET'&&url.pathname==='/health'){const models=await store.loadModels(),active=models.filter(m=>m.state==='Activa'),ready=active.filter(m=>!store.configurationProblem(m));return json(res,200,{ok:true,service:'Revisor Artículos API',engine:'hybrid-v3-microcriteria',database:true,models:models.length,active:active.length,ready:ready.length,providers:[...new Set(models.map(m=>m.provider))],time:new Date().toISOString()},origin)}
...
store.initDb().then(()=>server.listen(PORT,'0.0.0.0',()=>console.log(`Revisor Artículos API activa en ${PORT} con motor híbrido v3 por microcriterios y persistencia PostgreSQL`))).catch(err=>{console.error('No se pudo iniciar el backend:',err);process.exit(1)});
